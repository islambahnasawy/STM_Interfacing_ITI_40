...
static void update_Watch_Runnable(void);//1000ms
static void data_Extract(void);//mesh m3ana
static void sendFrame_Runnable(void);//50ms
static void receiveFrame_Runnable(void);//25ms
static void config_Watch_Runnable(void);//25ms
static void watch_App_Task(void);


#define Task_PERODICITY			25
#define UPDATE_WATCH_PERODICITY	1000
#define SEND_FRAME_PERODICITY	50
...
const task_t Watch_App_task = {.handler=watch_App_Task,.preodicityMS=Task_PERODICITY};
...
static void watch_App_Task(void)
{
	...
	send_counter ++;
	updateWatch_counter++;
	if(updateWatch_counter == (UPDATE_WATCH_PERODICITY/Task_PERODICITY))
	{
		updateWatch_counter=0;
		update_Watch_Runnable();
	}
	receiveFrame_Runnable();
	config_Watch_Runnable();
	if(send_counter == (SEND_FRAME_PERODICITY/Task_PERODICITY))
	{
		send_counter = 0;
		sendFrame_Runnable();
	}
...
static void sendFrame_Runnable(void)
{
...
static void receiveFrame_Runnable(void)
{
...
static void update_Watch_Runnable(void)
{
	/*Display*/
...
static void config_Watch_Runnable(void)
{
