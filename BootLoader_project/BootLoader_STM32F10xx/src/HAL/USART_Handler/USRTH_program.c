...
void USRTH_DMAStop(USART_t* USART)
{
	USART_DMAStop(USART);
}
...
{
	u8 local_status = OK;
	local_status = USART_DMASend(USART,Copy_Data,Copy_Length,handler);
	return local_status;
}
...
u8 USRTH_DMARecieve(USART_t* USART,u8* Copy_Data , u32 Copy_Length , Handler_t handler)
{
	...
	local_status = USART_DMARecieve(USART,Copy_Data,Copy_Length,handler);
